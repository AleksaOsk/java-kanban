...
public class TaskHttpHandler extends BaseHttpHandler {
    ...
    protected void handleGet(HttpExchange exchange, String type, String[] pathArray) throws IOException {
        if (pathArray.length == 3) {
            ...
            if (task != null) {
                ...
                if (type.equals("tasks")) {
                    ...
                    writeResponse("Такой подзади нет", exchange, 404);
                }
            }
        }
    }

    protected void handlePost(HttpExchange exchange, String type, String[] pathArray, Task task) throws IOException {
        if (pathArray.length == 3) {
    ...
    protected void handleDelete(HttpExchange exchange, String type, String[] pathArray) throws IOException {
        switch (type) {
            case "tasks" -> {
    ...
    protected List<? extends Task> getAll(String type) {
        switch (type) {
            case "tasks" -> {
    ...
    protected Task getOneOf(String type, int id) {
        return switch (type) {
    ...
    protected Task create(String type, Task task) {
        return switch (type) {
    ...
    protected Task update(String type, Task task) {
        return switch (type) {
